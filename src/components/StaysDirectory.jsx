          ...
          >
            <div className="flex items-center justify-between border-b border-slate-850 pb-2.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="px-2 py-1 bg-slate-900 hover:bg-slate-800 text-blue-300 border border-slate-700/80 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-all"
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-400 block font-semibold">Booking PNR / Ref</span>
              ...
              <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                <span className="text-[10px] text-slate-400 block font-semibold">Front Desk & Hotline</span>
                <a
                  href={"tel:" + (stay.phone || "")}
        ...
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveStay} className="space-y-3.5 text-xs">
              <div>
                <label className="text-slate-300 font-bold block mb-1">Select Hotel / Destination:</label>
                <select
                  value={selectedKey}
                  onChange={(e) => handleKeySelect(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 font-bold text-white text-xs outline-none focus:border-blue-500"
                >
                  {staysList.map((s) => (
              ...
              <div>
                <label className="text-slate-300 font-bold block mb-1">Hotel Name:</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-slate-300 font-bold block mb-1">Vietnamese Address (for Grab / Taxi Driver):</label>
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-slate-200 text-xs outline-none focus:border-blue-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  <label className="text-slate-300 font-bold block mb-1">Phone / Hotline:</label>
                  <input
                    type="text"
                    placeholder="e.g. +84 24 3926 3641"
                    value={form.phone}
                    onChange={(e) => setForm((prev) => ({ ...prev, phone: e.target.value }))}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 font-mono text-white text-xs outline-none focus:border-blue-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 font-mono font-bold text-emerald-400 text-xs outline-none focus:border-blue-500"
                  />
                </div>
              </div>

              <div>
                <label className="text-slate-300 font-bold block mb-1">Room Notes & Instructions:</label>
                <textarea
                  rows={2}
                  placeholder="e.g. 1 Deluxe Double + 1 Family Suite. Connecting rooms requested."
                  value={form.room}
                  onChange={(e) => setForm((prev) => ({ ...prev, room: e.target.value }))}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-slate-300 text-xs outline-none focus:border-blue-500 resize-none"
                />
              </div>
