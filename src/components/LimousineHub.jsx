  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 space-y-1 text-xs">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
        <div className="bg-slate-950 px-5 py-3 border-b border-slate-800 flex items-center justify-between text-xs font-bold text-slate-300">
          ...
          <span className="text-amber-400 font-mono text-[11px]">All-Inclusive Private Chauffeur</span>
        </div>
              ...
              {limoTransfers.map((leg, idx) => (
                <tr key={idx} className="hover:bg-slate-850/50 transition-colors">
                  ...
                  <td className="py-3 px-4 text-right whitespace-nowrap font-mono">
                    <strong className="text-emerald-400">{leg.costVnd}</strong>
                    <span className="text-[11px] text-slate-400 ml-1.5 font-normal">({leg.costInr})</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {isQuoteModalOpen && (
        <div className="fixed inset-0 z-[9999] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg p-6 space-y-4 shadow-2xl text-left">
              ...
              {limoTransfers.map((leg, idx) => (
                ...
                >
                  <div className="truncate">
                    <strong className="text-amber-400 mr-2">{leg.leg}:</strong>
                    <span>{leg.route}</span>
                  </div>
                  <input
                    type="checkbox"
